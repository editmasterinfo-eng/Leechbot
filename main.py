...
async def progress_for_pyrogram(current, total, ud_type, message, start_time):
    ...
    if round(diff % 5.00) == 0 or current == total:
        ...
        progress_text = (
            ...
            f"âž¢ **Size:** {humanbytes(current)} / {humanbytes(total)}\n"
            f"âž¢ **Speed:** {humanbytes(speed)}/s\n"
            f"âž¢ **ETA:** {time.strftime('%H:%M:%S', time.gmtime(eta))}"
        )
...
async def process_link(bot: Client, m: Message, url: str):
    ...
    - Cleans up
    """
    msg = await m.reply_text("ðŸ”Ž **Processing Link & Extracting Info...**", quote=True)
    ...
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                await msg.edit_text(f"âŒ Error getting link info: `{e}`")
                return

        # 2. Download Video
        await msg.edit_text(f"â¬‡ï¸ **Downloading:** `{final_title}`\n\nPlease wait...")
        ...
        if not os.path.exists(out_filename):
            ...
            for file in os.listdir('.'):
                if file.startswith(original_title): # Check with original title
                    ...
                    break
            if not found:
                await msg.edit_text("âŒ Download Failed. File not found after download.")
                return
        ...
        await m.reply_video(
            ...
            progress_args=("â¬†ï¸ Uploading...", msg, start_time)
        )
        ...
        await msg.edit_text(f"âŒ **An unexpected error occurred:**\n`{str(e)}`")
...
async def start_command(bot: Client, m: Message):
    await m.reply_text(
        f"ðŸ‘‹ Hello {m.from_user.first_name}!\n\n"
...
async def bulk_download(bot: Client, m: Message):
    ...
    await m.reply(f"âœ… **Bulk process started for {total_links} links.**\n\nMain ek-ek karke sabhi videos bhej dunga.", quote=True)
    ...
    for i, link in enumerate(links):
        ...
        try:
            ...
            await status_msg.edit(f"âŒ **Failed to process link {i+1}/{total_links}:**\n`{link}`\n\n**Error:** `{e}`")
            continue # Continue to the next link
    
    await m.reply("ðŸŽ‰ **Bulk process finished!**", quote=True)
