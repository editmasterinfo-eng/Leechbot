...
def get_system_stats():
    ...
    stats = f"**🖥️ CPU:** {cpu_usage}% | **🧠 RAM:** {ram_usage}%"
    try:
...
async def progress_bar(current, total, message_obj, start_time, status_text):
    ...
    if round(diff % 4.00)==0 or current==total:
        ...
        tmp = (f"{prog}**📦 Done:** {humanbytes(current)}/{humanbytes(total)}\n"
               f"**🚀 Speed:** {humanbytes(s)}/s\n"
...
**Khaas Features:**
🔹 **Google Drive & YouTube-DL Support.**
🔹 **Bulk Downloader:** `/bulk` se ek saath kai links download karein.
🔹 **Live Status:** Har download me live speed, progress aur system usage dekhein.
🔹 **Cancellation:** `/cancel` command se kisi bhi download ko rokein.
...
async def cancel_tasks_command(bot: Client, m: Message):
    ...
    if m.reply_to_message:
        ...
        if user_id in ACTIVE_TASKS and msg_id_to_cancel in ACTIVE_TASKS[user_id]:
            ...
            await m.reply_text(f"✅ Task `{msg_id_to_cancel}` ko cancel karne ka request bhej diya gaya hai.")
        else:
            await m.reply_text("❌ Is message se juda koi active task nahi mila.")
        return
    ...
    if user_id in ACTIVE_TASKS and ACTIVE_TASKS[user_id]:
        ...
        for task in ACTIVE_TASKS[user_id].values():
            task.cancel()
            count += 1
        await m.reply_text(f"✅ Aapke sabhi `{count}` active tasks ko cancel karne ka request bhej diya gaya hai.")
    else:
        await m.reply_text("🤷‍♂️ Aapka koi bhi task active nahi hai.")
...
async def process_link(client: Client, m: Message, url: str, status_msg: Message):
    ...
    try:
        ...
        if "drive.google.com" in url:
            ...
            await edit_status(status_msg, f"⬇️ Downloading: `{safe_title}`")
...
async def bulk_download(bot: Client, m: Message):
    ...
    if not links: return await m.reply_text("❓ Diye gaye text me koi valid link nahi mila.")
